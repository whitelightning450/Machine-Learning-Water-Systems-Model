...
class XGB_model():
    ...
    def XGB_Predict(self, test_feat, test_targ):
        ...
        model = pickle.load(open("Models/V2/XGBoost_"+self.Target+".dat", "rb"))
        start_time = time.time()  
...
class XGB_Prediction():
    ...
    def __init__(self, MDell_Thresh, LDell_Thresh, units, Sim, condition, test_yr, figsave):
        self = self
    ...
    def WSM_Predict(self):   
        ...
        self.MDell_model = pickle.load(open("Models/V1/XGBoost_"+self.MDell+".dat", "rb"))
        self.LDell_model = pickle.load(open("Models/V2/XGBoost_"+self.LDell+".dat", "rb"))
        self.GW_model = pickle.load(open("Models/V2/XGBoost_"+self.GW+".dat", "rb"))
        self.DC_model = pickle.load(open("Models/V2/XGBoost_"+self.DC+".dat", "rb"))
 ...
 #Perform a historical analysis of each WSC to compare performance of current scenario       
    ...
    def WSM_Pred_RRV_Plot(self):
        ...
        if self.obs ==True:
            ...
            ax[0,1].errorbar(self.Analysis[self.MDell], self.Analysis[self.MDell_Pred], yerr = asymmetric_error_MDell, fmt='.k', ecolor = 'slateblue', mec = 'blue') 
            ...
            ax[0,2].bar(self.components[self.MDell].index, components[self.MDell]['XGB_WSM'], yerr = asymmetric_error_RRV_MDell, capsize = 4, width = width, align = 'edge', color = 'blue')
            ax[0,2].bar(self.components[self.MDell].index, self.components[self.MDell]['SLC_WSM'], width = -width, align = 'edge', color = 'red')
            ...
            #LDell parity
            ax[1,1].errorbar(self.Analysis[self.LDell], self.Analysis[self.LDell_Pred], yerr = asymmetric_error_LDell, fmt='.k', ecolor = 'slateblue', mec = 'blue') 
            ...
            # GW parity
            ax[2,1].errorbar(self.Analysis[self.GW], self.Analysis[self.GW_Pred], yerr = asymmetric_error_GW, fmt='.k', ecolor = 'slateblue', mec = 'blue') 
            ...
            #Parity plot
            ax[3,1].errorbar(self.Analysis[self.DC], self.Analysis[self.DC_Pred], yerr = asymmetric_error_DC, fmt='.k', ecolor = 'slateblue', mec = 'blue') 
        ...
        if self.figsave == True:
            ...
            fig.savefig('Figures/'+self.scenario+ '_Analysis.png', dpi =300)
